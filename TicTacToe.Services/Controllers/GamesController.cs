namespace TicTacToe.Services.Controllers
{
    ...
    public class GamesController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/games/state")]
        [AllowAnonymous]
        public IHttpActionResult ChangeGameState(int id)
        {
            var searchedGame = this._data.Games.FirstOrDefault(g => g.Id == id);
            searchedGame.State = GameState.NoWinner;
            this._data.SaveChanges();
            return this.Ok(searchedGame.State.ToString());
        }

        [HttpPost]
        [Route("api/games")]
        public IHttpActionResult EditGame(EditGameBindingModel model)
        {
            var searchedGame = this._data.Games.FirstOrDefault(g => g.Id == model.Id);
            searchedGame.Field = model.Field;
            searchedGame.State = GameState.NoWinner;
            searchedGame.Name = model.Name;
            this._data.SaveChanges();
            return this.Ok();
        }
